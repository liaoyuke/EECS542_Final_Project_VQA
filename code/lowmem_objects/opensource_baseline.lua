...
function build_model(opt, manager_vocab) 
    ...
    elseif opt.method == 'BOWIMG_C' then
        ...
        local module_tdata = nn.Sequential():add(nn.SelectTable(1)):add(nn.LinearNB(manager_vocab.nvocab_question, opt.embed_word))
        local module_vdata = nn.Sequential():add(nn.SelectTable(2))
        local module_cdata = nn.Sequential():add(nn.SelectTable(3)):add(nn.LinearNB(manager_vocab.nvocab_objects, opt.embed_word))
...
function initial_params()
    ...
    cmd:option('--thresh_objectsword', 1, 'threshold for the word freq on the concept')
...
function runTest()
    ...
    local method = 'BOWIMG_C'
    local model_path = 'model/BOWIMG_C.t7'

...
function load_visualqadataset(opt, dataType, manager_vocab)
    ...
    ---------------------------
    local filename_cca = paths.concat(path_dataset, prefix .. '_gt_CCA.dat')
    local featCCA = torch.load(filename_cca)
...
function train_epoch(opt, state, manager_vocab, context, updateIDX)
    ...
    local featBatch_cca = torch.zeros(opt.batchsize, opt.cdim)
    ...
    for iii = 1, state.x_question:size(1) do
        ...
        if first_answer == -1 and updateIDX == 'train' then
            ...
            local feat_cca = state.featCCA[i]:clone()
            ...
            featBatch_cca[nSample_batch] = feat_cca:clone()
            ...
            if nSample_batch == opt.batchsize then                
                ...
                featBatch_cca = featBatch_cca:cuda()
                ----------forward pass----------------------
                ...
                elseif opt.method == 'BOWIMG_C' then
                    input = {featBatch_word, featBatch_cca, featBatch_visual}

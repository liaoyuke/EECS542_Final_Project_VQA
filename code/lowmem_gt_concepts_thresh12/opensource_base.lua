...
function load_visualqadataset(opt, dataType, manager_vocab)
    ...
    local filename_concept = paths.concat(path_dataset, prefix .. '_gt_concepts.txt')
    ...
    print("Load concept file = " .. filename_concept)
    local data_concept = file.read(filename_concept)
    local data_concept_split = stringx.split(data_concept,'\n')
    ...
    if manager_vocab == nil then
        ...
        local vocab_map_concept, ivocab_map_concept, nvocab_concept = build_vocab(data_concept,opt.thresh_conceptword, 0, 1)
    ...
    -- Convert words in answers and questions and concepts to indices into the dictionary.
    local x_question = torch.zeros(nSample, opt.seq_length)
    local x_concept = torch.zeros(nSample, opt.seq_length)
    for i = 1, nSample do
        ...
        -- Concepts
        local concept_words = stringx.split(data_concept_split[i])
        for j = 1, opt.seq_length do
            if j <= #concept_words then
                if manager_vocab_.vocab_map_concept[concept_words[j]] == nil then
                    x_concept[{i, j}] = 1
                else
                    x_concept[{i, j}] = manager_vocab_.vocab_map_concept[concept_words[j]]
                end
            else
                x_concept[{i, j}] = manager_vocab_.vocab_map_concept['END']
            end
        end
    end
...
function bagofword_concept(manager_vocab, x_seq)
    local outputVector = torch.zeros(manager_vocab.nvocab_concept)
    for i= 1, x_seq:size(1) do
        if x_seq[i] ~= manager_vocab.vocab_map_concept['END'] then    
            outputVector[x_seq[i]] = 1
        else
            break
        end
...
function train_epoch(opt, state, manager_vocab, context, updateIDX)
    ...
    local prob_answer = nil
    local pred_answer = nil
    if (updateIDX == 'test') then
        prob_answer = torch.zeros(state.x_question:size(1), manager_vocab.nvocab_answer)
        pred_answer = torch.zeros(state.x_question:size(1))
    end
    ...
    local featBatch_concept = torch.zeros(opt.batchsize, manager_vocab.nvocab_concept)
    ...
    for iii = 1, state.x_question:size(1) do
        ...
        if first_answer == -1 and updateIDX == 'train' then
            ...
            local feat_concept = bagofword_concept(manager_vocab, state.x_concept[i])
            ...
            featBatch_concept[nSample_batch] = feat_concept:clone()
            ...
            if nSample_batch == opt.batchsize then                
                ...
                featBatch_concept = featBatch_concept:cuda()
                ----------forward pass----------------------
